...
export const AuthShell = ({ eyebrow, title, description, children }: AuthShellProps) => {
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#f6f8fb] px-4 py-3 text-slate-950 sm:px-6 sm:py-4">
      ...
      <section className="relative w-full max-w-[500px] animate-[auth-card-enter_560ms_cubic-bezier(0.22,1,0.36,1)_both]">
        ...
        <div className="relative overflow-hidden rounded-[34px] border border-white/80 bg-white/95 shadow-[0_28px_90px_rgba(15,23,42,0.14)] backdrop-blur">
          ...
          <div className="px-6 pb-5 pt-6 sm:px-9 sm:pb-6 sm:pt-7">
            <header className="text-center">
              <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-[24px] border border-slate-200 bg-[#f8fafc] shadow-[inset_0_1px_0_rgba(255,255,255,0.85),0_12px_28px_rgba(15,23,42,0.08)]">
                <img src={dtsLogo} alt="DTS Logo" className="h-16 w-16 object-contain" />
              </div>

              {eyebrow && (
                <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-[#006482]">
                  {eyebrow}
                </p>
              )}
              <h1 className="mt-2 text-[30px] font-semibold leading-tight tracking-[-0.01em] text-slate-950">
                {title}
              </h1>
              <p className="mx-auto mt-2 max-w-[360px] text-sm leading-5 text-slate-500">
                {description}
              </p>
            </header>
        ...
        <footer className="mt-3 text-center text-xs leading-4 text-slate-500">
          <p>© 2026 DTS</p>
